...
class Solution {
    public List<Integer> fallingSquares(int[][] positions) {
        ...
        for (int[] position : positions){
            if (!height.containsKey(position[0])){
                index.add(position[0]);
            }
            if (!height.containsKey(position[0]+position[1]-1)){
                index.add(position[0]+position[1]-1);
            }
            height.put(position[0], 0);
            height.put(position[0]+position[1]-1, 0);
        }
        ...
        for (int[] position : positions){
            ...
            for (int i=left; i <= right; i++){
                currHeight = Math.max(currHeight, height.get(index.get(i)));
            }
            currHeight += position[1];
            for (int i=left; i <= right; i++){
                height.put(index.get(i), currHeight);
            }
            curr = Math.max(curr, currHeight);
            result.add(curr);
        }
