...
class Solution {
    public String simplifyPath(String path) {
        ...
        for (; end < path.length(); end++) {
            if (path.charAt(end) == '/') {
                if (start+1 < end)
                    pushStack(stack, path.substring(start+1, end));
                start = end;
            }
        }
        if (start+1 < path.length())
            pushStack(stack, path.substring(start+1));
        while (!stack.isEmpty()){
    ...
    private void pushStack(Stack<String> stack, String tem) {
        if (tem.equals("..") && !stack.isEmpty())
            stack.pop();
