...
class Solution {
    public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
        ...
        for (int i = 0; i < equations.size(); i++){
            merge(father, weight, variable.get(equations.get(i).get(0)), variable.get(equations.get(i).get(1)), values[i]);
        }
        for (int i = 0; i < n; i++){
            if (!variable.containsKey(queries.get(i).get(0)) || !variable.containsKey(queries.get(i).get(1))){
                result[i] = -1.0;
            }else if (findFather(father, weight, variable.get(queries.get(i).get(0))) == findFather(father, weight, variable.get(queries.get(i).get(1)))){
                result[i] = weight[variable.get(queries.get(i).get(0))] / weight[variable.get(queries.get(i).get(1))];
            }else{
                result[i] = -1.0;
            }
        }
    ...
    private int findFather(int[] father, double[] weight, int x){
        if (father[x] != x){
    ...
    private void merge(int[] father, double[] weight, int a, int b, double value){
